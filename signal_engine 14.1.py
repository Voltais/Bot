"""
SIGNAL ENGINE V14.1 – с диагностикой причин NO_TRADE
- Добавлено поле rejection_stage в TradeSetup
- Все отказы в swing_signal_v10_6 теперь возвращают структурированный TradeSetup с причиной
- В generate_context выводится причина NO_TRADE
- Контртрендовая логика V14.0 сохранена (работает без жёсткого range_market)
"""
...
class TradeSetup:
    ...
    hold_note: str = ""
    rejection_stage: str = ""   # НОВОЕ ПОЛЕ: диагностика причины NO_TRADE
...
def compute_poi_confluence(state: MarketState, bias: str, candles_4h: List[Tuple[float, float, float, float, float]]) -> Tuple[Optional[float], float]:
    ...
    # A POI must come from an actual SMC source: liquidity, order block, or FVG.
    if not candidates:
        return None, 0.0
...
def swing_signal_v10_6(state: MarketState, candles_4h: List[Tuple[float, float, float, float, float]], candles_15m: List[Tuple[float, float, float, float, float]]) -> Optional[TradeSetup]:
    ...
    if bias == "bullish":
        ...
        return TradeSetup("LONG", round_to_tick(state.symbol, entry_price), round_stop(state.symbol, stop, "LONG"), round_to_tick(state.symbol, tp1), round_to_tick(state.symbol, tp2), final_conf, "V14.1 трендовый сигнал (bullish)", "SWING", pd_4h, sweep_detected, entry_type, is_counter_trend=False)
    else:
        stop = poi + atr * Risk.STRUCTURE_BUFFER_ATR
        stop = enforce_min_stop(entry_price, stop)
        min_dist = max(state.price * Risk.MIN_LIQ_DISTANCE_PERCENT, state.atr_4h * Risk.MIN_LIQ_DISTANCE_ATR_MULT)
        ...
        return TradeSetup("SHORT", round_to_tick(state.symbol, entry_price), round_stop(state.symbol, stop, "SHORT"), round_to_tick(state.symbol, tp1), round_to_tick(state.symbol, tp2), final_conf, "V14.1 трендовый сигнал (bearish)", "SWING", pd_4h, sweep_detected, entry_type, is_counter_trend=False)
...
def generate_context(state: MarketState, swing: TradeSetup, regime: str, gtrend: str, is_active: bool = False, age_hours: float = 0.0) -> str:
    ...
    # НОВОЕ: вывод диагностики NO_TRADE
...
def generate_signals(state: MarketState, candles_4h: List[Tuple[float, float, float, float, float]] = None, candles_15m: List[Tuple[float, float, float, float, float]] = None) -> MultiSignal:
    now = time.time()
    symbol = state.symbol
    logger.info(f"GENERATE_SIGNALS: symbol={symbol}, price={state.price:.2f} (V14.1)")
    ...
    # Исправлено: new_setup теперь всегда TradeSetup (даже для NO_TRADE с диагностикой)
    if new_setup and new_setup.direction != "NO_TRADE":
        ...
        # Использовать new_setup (с диагностикой) если он есть, иначе пустой
