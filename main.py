...
_futures_rate_limiter = AsyncRateLimiter(calls_per_minute=20 * 60)  # можно увеличить до 40*60
...
async def _call_model(model_name: str, timeout: int, chat_context: Any) -> Optional[str]:
    ...
    data = {
        ...
        "thinking": {"type": "disabled"},  # Задача модели — только оформление уже готовых данных,
                                            # рассуждать не над чем; thinking-режим только жжёт токены
                                            # и при усложнении промпта может съесть весь max_tokens
                                            # до появления content, оставляя пустой финальный ответ.
    }
        ...
        try:
            async with http_client.stream("POST", "https://api.deepseek.com/v1/chat/completions", json=data, headers=headers, timeout=timeout) as response:
                ...
                if text:
                    ...
                    return text
                # ВАЖНО: reasoning_content — это внутренний ход мыслей модели, а не готовый
                # ответ. Никогда не показывать его пользователю. Если content пуст (модель не
                # успела/не смогла сформировать финальный ответ) — считаем это неудачей вызова,
                # чтобы сработал штатный фолбэк на другую модель / сообщение об ошибке.
                if reasoning_content.strip():
                    logger.warning(f"{model_name}: получен только reasoning_content без content (обрезано? см. max_tokens) — ответ отклонён, не показан пользователю")
                return None
...
# Включаются только если основной источник реально недоступен (сеть, техработы,
# гео-блокировка) — не подменяют и не отменяют ни рейт-лимитер, ни кэш (@cached
# на get_spot_data/get_futures_data работает как и раньше), ни обработку
# "тикер не найден" (ValueError пробрасывается сразу, без fallback, потому что
# другая биржа не найдёт несуществующий тикер тоже).
...
async def get_spot_data(symbol: str):
    try:
        ...
        for tf in TIMEFRAMES:
            ...
            last_values[tf] = last
            # Сохраняем EMA для нужных таймфреймов
            if tf == "1h":
...
async def get_combined_analysis(symbol: str):
    ...
    if (fut_signal.swing.direction == spot_signal.swing.direction and 
        ...
        if fut_signal.swing.confidence > 0.6:
            ...
            fut_signal.swing.direction = "WAIT"
            if hasattr(fut_signal, 'context') and fut_signal.context:
...
async def inactive_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Список неактивных (истёкших) подписчиков. Только отчётность — физически
    никого не удаляет. Помечает 🗑️ тех, кто дольше INACTIVE_USERS_RETENTION_DAYS
    без подписки, и предупреждает, если общее число превышает INACTIVE_USERS_LIMIT.
    Фактическое удаление — отдельное решение на будущее.
    """
    if update.effective_user.id != OWNER_ID:
        await update.message.reply_text("⛔ Доступ запрещён")
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id == OWNER_ID or user_id in EXEMPT_USERS:
        # Владелец и вручную добавленные пользователи никогда не платят и не проходят
        # через activate_subscription (единственную точку создания users-записи для
        # подписчиков) — но они всё равно пишут в messages/daily_requests/signal_outcomes,
        # которые ссылаются на users по внешнему ключу. Поэтому только для этой группы
        # гарантируем существование строки перед первой такой записью.
    ...
    try:
        ...
        if ticker:
            ...
            # Функция форматирования сигнала для промпта (без маркдауна, просто текст)
...
"""
        ...
        def split_into_chunks(text: str, limit: int = 4096) -> list:
            """Режет по границам строк, а не вслепую по индексу — не разрывает HTML-теги пополам."""
            if len(text) <= limit:
                return [text]
            chunks, current = [], ""
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "subscription":
        ...
        # create_invoice_record пишет invoices.user_id (FK на users) ДО оплаты и ДО
        # activate_subscription — поэтому здесь, и только здесь, нужно заранее
        # гарантировать существование users-записи для потенциального нового плательщика.
...
# Небольшой (необязательный) список самых популярных пар, которые прогреваются
# заранее, чтобы ответ для большинства пользователей был мгновенным. Любая другая
# пара по-прежнему запрашивается и кэшируется по требованию, как и раньше —
# это ДОПОЛНЕНИЕ к обычному пути, а не замена его.
POPULAR_SYMBOLS = ["BTC/USDT", "ETH/USDT"]

async def popular_pairs_prewarmer(interval_seconds: int = 45):
    """
    Фоновый прогрев кэша (interval_seconds меньше TTL кэша = 60с, поэтому
    для этих пар кэш почти никогда не успевает "остыть"). Использует те же
    get_spot_data/get_futures_data и тот же cache_manager, что и обычный
    пользовательский путь — отдельного параллельного хранилища нет.
    """
...
def _load_active_signals_from_db() -> None:
    """Восстанавливает активные сигналы движка из БД при старте бота (после рестарта/деплоя),
    затем подключает запись изменений обратно в БД (write-through)."""
    restored = []
    try:
